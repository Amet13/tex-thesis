...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Inputs
        \node[lbl, left=2.2cm of sys, yshift=0.7cm] (in1) {Raw data};
        \node[lbl, left=2.2cm of sys] (in2) {Configuration};
        \node[lbl, left=2.2cm of sys, yshift=-0.7cm] (in3) {User requirements};

        % Outputs
        \node[lbl, right=2.2cm of sys, yshift=0.5cm] (out1) {Processed results};
        \node[lbl, right=2.2cm of sys, yshift=-0.5cm] (out2) {Performance metrics};
...
\cref{fig:state-machine} shows the state transitions of the data processing pipeline.
...
The resource allocation across subsystems is shown in \cref{fig:pie-chart}.
