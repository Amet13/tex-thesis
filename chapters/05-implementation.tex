...
Figure~\ref{fig:sample-photo} demonstrates including an external image from the \texttt{images/} directory using the \verb|\addimghere| custom command.
...
Figure~\ref{fig:two-images} shows two images placed side by side using the \verb|\addtwoimghere| command.
...
Listing~\ref{lst:processor} shows a simplified version of the data processor.
...
Listing~\ref{lst:config} shows an example configuration.
...
Listing~\ref{lst:sql} shows the database schema used for storing experimental results.
...
Listing~\ref{lst:dockerfile} shows the Dockerfile used for containerized deployment.
...
The system implements several security measures, as shown in Figure~\ref{fig:security}.
